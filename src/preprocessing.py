    ...
    with open(features_path / feature_file, "r") as file:
        ...
        for _ in range(201):
            ...
            if img_info[479].find(".db") != -1:
                continue
            img_features = [int(feature) for feature in img_info[0:80] + img_info[336:478]]
...
with open("unnorm.txt", "w") as file:
    for img in all_imgs_features:
        ...
        for feature_idx in range(len(all_imgs_features[img][1])):
            ...
            all_imgs_features[img][1][feature_idx] = (all_imgs_features[img][1][feature_idx] - min_feature) / (max_feature - min_feature)
        print(file=file)
